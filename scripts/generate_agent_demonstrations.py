...
def create_scenarios():
    ...
    return [
        (s1_user, s1_job, "Scenario 1: Arabic/Visual Accessibility"),
...
def format_as_markdown(results, output_path):
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Recommended Courses**: {', '.join(results['ups'].recommendations[0].get('suggested_courses', [])) if results['ups'].recommendations else 'None'}\n\n"
        )

        f.write("### 🏠 Accommodation Agent\n")
        f.write(
            f"- **Strategy**: {results['acc'].recommendations[0].get('strategy', 'N/A') if results['acc'].recommendations else 'N/A'}\n"
        )
        f.write(
            f"- **Cost Est**: {results['acc'].recommendations[0].get('estimated_cost', 'N/A') if results['acc'].recommendations else 'N/A'}\n\n"
        )
        ...
        f.write(
            f"- **Status**: {'🚩 FLAG FOR HUMAN REVIEW' if results['gov'].risk_score > 0.4 else '✅ APPROVED'}\n"
        )
