...
def main():
    ...
    for lam_config in sorted(Path("configs/experiment/lambda_sweep").glob("*.yaml")):
        ...
        for seed in seeds:
            ...
            if final_json.exists():
                ...
                d_vals.append(final.get("final_round_metrics", {}).get("mean_fairness_disparity_disability", float("nan")))
            else:
                ...
                if per_round_json.exists():
                    ...
                    if history:
                        ...
                        d_vals.append(history[-1].get("mean_fairness_disparity_disability", float("nan")))
    ...
    scatter = ax.scatter(df["d_mean"], df["f1_mean"], 
                         c=df["lambda"], cmap="viridis", s=100, zorder=5, 
                         edgecolor='white', alpha=0.9, linewidth=1.5)

    # Error bars for both dimensions
    ax.errorbar(df["d_mean"], df["f1_mean"], 
                xerr=df["d_std"], yerr=df["f1_std"],
                fmt='none', ecolor='gray', alpha=0.4, capsize=0, zorder=2)
