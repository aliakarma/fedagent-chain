...
def find_latest_run(runs_dir: Path) -> Path | None:
    runs = sorted(
        runs_dir.glob("fedagent_chain_full_*"), key=lambda p: p.stat().st_mtime, reverse=True
    )
...
def main():
    ...
    plt.bar(
        rounds,
        bc_times,
        bottom=np.array(local_times) + np.array(agg_times),
    ...
    overhead_df = pd.DataFrame(
        [
            ...
            (
                "Model payload size",
                f"{history[0]['model_size_kb']:.0f} KB",
    ...
    comm_df = pd.DataFrame(
        {"Round": rounds, "Bytes_Transmitted": bytes_transmitted, "Cumulative_MB": cumulative_mb}
    )
